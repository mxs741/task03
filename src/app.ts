...
const normalizeData: Function = (unnormalizedData: Post[]): NormalizedData => {
  const result: NormalizedData = {byId: {}, allIds: unnormalizedData.map((post: Post) => post.id)}
  unnormalizedData.forEach((post: Post): void => {
    result.byId[post.id] = {'id': post.id, 'title': post.title, 'body': post.body}
  })
  return result
};
